...
int test_convert_uyvy_i420() {
    ...
    /* Set up test data's path and output data's path */
    const char *inputPathname  = "./11_1920x1080.UYVY";
    const char *outputPathname = "./11_1920x1080.I420";
    FILE *fin  = fopen(inputPathname , "rb+");
    if (!fin) {
        return -1;
    }
    ...
    fclose(fin);
    unsigned char *dest_img = (unsigned char *)malloc(dest_size * sizeof(unsigned char));
    std::cout << G_IMAGE_UTILITY.convert_uyvy_i420(src_img, pic_w, pic_h, dest_img) << std::endl;
    free(src_img);
    /* Write data of buf to fout */
    fwrite(dest_img, dest_size, 1, fout);
    /* Close the file */
...
int test_convert_i420_uyvy() {
    ...
    /* Set up test data's path and output data's path */
    const char *inputPathname  = "./11_1920x1080.I420";
    const char *outputPathname = "./11_1920x1080.UYVY";
    FILE *fin  = fopen(inputPathname , "rb+");
    if (!fin) {
        return -1;
    }
    ...
    fclose(fin);
    unsigned char *dest_img = (unsigned char *)malloc(dest_size * sizeof(unsigned char));
    std::cout << G_IMAGE_UTILITY.convert_i420_uyvy(src_img, pic_w, pic_h, dest_img) << std::endl;
    free(src_img);
    /* Write data of buf to fout */
    fwrite(dest_img, dest_size, 1, fout);
    /* Close the file */
